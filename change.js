...
function setChange() {
 change = (oneCent * 0.01) + (fiveCent * 0.05) + (tenCent * 0.1) + (twentyfiveCent * 0.25) + (fiftyCent * 0.5) + oneDollar + (
   twoDollar * 2) + (fiveDollar * 5) +
  (tenDollar * 10);
 change = change.toFixed(2);
 document.getElementById("oneC")
  .innerHTML = oneCent;
 document.getElementById("fiveC")
  .innerHTML = fiveCent;
 document.getElementById("tenC")
  .innerHTML = tenCent;
 document.getElementById("twentyfiveC")
  .innerHTML = twentyfiveCent;
 document.getElementById("fiftyC")
  .innerHTML = fiftyCent;
 document.getElementById("oneD")
  .innerHTML = oneDollar;
 document.getElementById("twoD")
  .innerHTML = twoDollar;
 document.getElementById("fiveD")
  .innerHTML = fiveDollar;
 document.getElementById("tenD")
  .innerHTML = tenDollar;
 if (!hard) {
...
function setRound() {
 if (lives > 0 && points < goal) {
  ...
  document.getElementById("payment")
   .innerHTML = paying;
  change = oneCent = fiveCent =
   tenCent = twentyfiveCent =
   fiftyCent = oneDollar = twoDollar =
   fiveDollar = tenDollar = 0;
  setChange();
 }
 if (points >= goal) {
  document.getElementById("feedback")
...
function startGame() {
 if (!document.getElementById("easy")
  .checked && !document.getElementById(
   "medium").checked && !document
  .getElementById("hard").checked) {
  alert(
   "Pick a difficulty level before starting"
  );
  return;
 }
 ...
 document.getElementById("feedback")
  .innerHTML =
  "New game started. Attend the first costumer in line." +
  setCostumer();
 document.getElementById("points")
  .innerHTML = "Points: " + points +
  "&MediumSpace; Goal:" + goal;
 setRound();
}

function returnChange() {
 let correctAnswer = paying - price;
 correctAnswer = correctAnswer.toFixed(
  2);
 if (lives > 0) {
  if (change == correctAnswer) {
   document.getElementById("feedback")
    .innerHTML =
    "CORRECT! Attend the next costumer in line." +
    setCostumer();
   points += (1 + difficulty);
   document.getElementById("points")
    .innerHTML = "Points: " + points +
